...
bool SwerveModule::SetDesiredState(const frc::SwerveModuleState &state, bool autonomous, double distance, double maxVelocity)
{
    ...
    if (!autonomous) {
        m_drivePIDController.SetReference(stateSpeed * speedFlip * 400, rev::ControlType::kVelocity); //*435
    ...
    //added all of this to try to get speedflip to work
    /*if ((current - command) >= 4.5) //&& flipping == 0)
    {
        while ((current - command) >= 4.5)
        {
            command += 9;
            if (flipping == 0)
            {
    ...
    else if ((current - command) <= 4.5) //&& flipping == 0)
    {
        while ((current - command) <= 4.5)
        {
            command -= 9;
    ...
    else if (abs(current - command) && flipping == 1)
    {
        flipping = 0;
    } */
